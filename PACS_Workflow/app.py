...
fig, ax = plt.subplots(figsize=(15, 12))  # taller figure

# Layout parameters
x_positions = [0.2, 0.5, 0.8]
category_titles = list(vendors.keys())
y_start = 1.0
y_spacing = 0.15   # larger spacing to spread things out
...
for col, (cat, vendor_list) in enumerate(vendors.items()):
    for row, (vendor, tag) in enumerate(vendor_list):
        ...
        # Blue cloud
        cloud = Ellipse((x, y), width=0.28, height=0.12, facecolor="#cfe8ff", edgecolor="blue", linewidth=1)
        ax.add_patch(cloud)
        ax.text(x, y + 0.08, vendor, ha="center", va="bottom", fontsize=9, color="navy", weight="bold")

        # Archive
        archive = Rectangle((x-0.06, y-0.025), 0.12, 0.05, facecolor="#FFD700", edgecolor="black", linewidth=1)
        ...
        # Connect to hospital
        ax.add_line(mlines.Line2D([x, 0.5], [y-0.06, -0.08], color="gray", linestyle="--", linewidth=1))

    # Category label
    ax.text(x, 1.08, cat, ha="center", va="bottom", fontsize=12, weight="bold")

# Hospital
hospital = Rectangle((0.35, -0.15), 0.3, 0.1, facecolor="#90EE90", edgecolor="black", linewidth=1.5)
ax.add_patch(hospital)
ax.text(0.5, -0.10, "Hospital", ha="center", va="center", fontsize=13, weight="bold")
...
- Patient priors are fragmented across **modality- and vendor-specific silos**.  
- Radiologists waste time, and IT faces brittle integrations.  
""")
